...
export const useGetAtmosphericPrevisionData = (params: Position | null ) => {
  return useQuery({
    queryKey: ['previsionsAirData', params?.latitude, params?.longitude],
    queryFn: () => {
      if (!params) {
        return Promise.reject(new Error('params is null'));
      }
      return fetcher(`/atmospheric/previsions/user/?latitude=${params.latitude}&longitude=${params.longitude}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
